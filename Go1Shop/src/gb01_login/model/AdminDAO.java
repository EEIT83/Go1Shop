...
public class AdminDAO implements AdminFace {
	...
	private StringBuilder UPDATE = new StringBuilder("update admin set ");
	...
	public void insert(AdminBean AdVo) throws Exception {
		try {
			if (AdVo != null) {
				Object params[] = { AdVo.getAd_mail(), AdVo.getAd_pwd(), AdVo.getCompetence() };
	...
	public void update(AdminBean AdVo) throws Exception {
		...
		if (StringUtils.isNoneEmpty(AdVo.getAd_pwd())) {
			UPDATE.append("ad_pwd=?, ");
			adList.add(AdVo.getAd_pwd());
		}

		UPDATE.delete(UPDATE.length() - 2, UPDATE.length());
		UPDATE.append(" where mail=? ");
		adList.add(AdVo.getAd_mail());
...
//		StringBuilder sb = new StringBuilder("select * from admin where competence > 1");
